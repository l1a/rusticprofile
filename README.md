...
cd rusticprofile
just install               # this checkout: binary + man page + completions for six shells
...
binary beside a man page **eleven releases old**, because every upgrade had been a hand-typed
`cargo install --git --tag` and nothing re-ran the other two recipes or reported that they were
stale — the same shape as a generated systemd unit that no upgrade re-emits.

It takes care to make all three artefacts agree, which is the part worth knowing: the binary comes
from the tag, the **completions are generated by the installed binary** so they cannot disagree
with its CLI, and the **man page is read out of the tag** (`git show v<version>:…`) rather than out
of your working tree. Installing a v0.2.22 binary next to a v0.2.23 man page because the checkout
had moved on is exactly the kind of individually-plausible mismatch this tool exists to refuse.
