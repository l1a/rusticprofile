...
**--background**
:   Mark the run as unattended, i.e. started by the scheduler rather than typed. **schedule** puts this in every generated Task Scheduler definition and in every generated systemd service, and it does two things — one on every platform and one only on Windows.

    Everywhere, it enables the **retry**: a failed operation is attempted twice more at two-minute intervals before the job stops. This exists because a missed calendar time is replayed within *seconds* of the machine waking, which is before the network is back, so the run that replaces a missed hour is the one most likely to fail. Measured on both platforms; on Linux two catch-ups each fired in the same second as a resume and failed on DNS, while the network became usable 11 and 12 seconds later. Only a plain failure is retried — an interrupted run is a decision, a partial one already continues to **forget**, and **--dry-run** never waits. The retry is stated in the operation's summary, so a retried run cannot read as a first-attempt success. A run you type yourself is unaffected and fails immediately, which is the whole reason this is a flag rather than something inferred: on Linux `INVOCATION_ID` and `JOURNAL_STREAM` are both set in an ordinary terminal, so the environment cannot distinguish the two cases.

    On **Windows only**, it additionally detaches from the console so the run shows no window. Task Scheduler can run a task as the logged-on user only through an interactive logon, which starts it inside that desktop session, where Windows gives a console program a console; the logon types that run outside it need rights an ordinary account does not hold. Typing it by hand there discards the run's output — rustic's progress goes nowhere — while the record still reaches the **log:** file. On Unix it touches no stream, so a scheduled run's diagnostics still reach the journal.
...
On **Windows** `next run` is rendered into that same shape too, and reads `Wed 2026-08-12 11:02:28 PDT (±5 min)`. Two details behind it are worth knowing, because both are properties of Task Scheduler rather than of this tool. The instant is obtained from `Get-ScheduledTaskInfo` via Windows PowerShell, because the value `schtasks` prints is formatted in the account's locale and `8/12/2026` means two different dates depending on it — so that string is never parsed, and where the lookup is unavailable the line falls back to printing it verbatim rather than risking a wrong date. And `(±5 min)` is the schedule's own randomised spread, which Task Scheduler **re-rolls on every query**: two `status` invocations seconds apart legitimately report times a few minutes apart, so the window is stated rather than implying a precision that is not there. `status --json` is unaffected — `next_run` stays exactly what the service manager reported. The window is not printed on Linux or macOS, where no such re-roll has been measured.
