...
/// A scratch home directory shared by every child this file spawns.
///
/// **The third member of the set `state_dir` and `runtime_dir` start**, added with `doctor`'s
/// `units-current` check (`0.2.48`). That check reads the installed units back from the default
/// unit directory — `~/.config/systemd/user` and `~/Library/LaunchAgents` both derive from
/// `$HOME` — and the fixtures name the fleet's live job, `dot-files`. So without this, the first
/// fixture to declare a `schedule:` would have compared against the developer's real, live
/// unit, and passed or failed on the state of the machine rather than the code. The existing
/// `doctor` fixtures were safe only because none declares a schedule, which is a coincidence and
/// not a guarantee.
///
/// Windows needs nothing here: its task definitions live under `XDG_STATE_HOME`, already
/// redirected, and `dirs::home_dir()` there does not read `HOME` at all.
fn home_dir() -> &'static std::path::Path {
    use std::sync::OnceLock;
    static DIR: OnceLock<tempfile::TempDir> = OnceLock::new();
    DIR.get_or_init(|| tempfile::tempdir().expect("scratch home dir"))
        .path()
}

/// The binary, with state, locks and the home directory redirected away from the developer's
/// real ones.
///
...
/// `doctor`'s `units-current` finding, run with `HOME` pointed at `home`.
#[cfg(target_os = "linux")]
fn units_current(home: &std::path::Path, config: &str, extra: &[&str]) -> (String, String, i32) {
    let mut args = vec!["doctor", "--config", config, "--json"];
    ...
    let finding = value["findings"]
        ...
        .expect("findings")
        .iter()
        .find(|f| f["check"] == "units-current")
        .unwrap_or_else(|| panic!("no units-current finding:\n{stdout}"))
        .clone();
    (
        finding["severity"].as_str().unwrap().to_string(),
        finding.to_string(),
        output.status.code().expect("exit"),
    )
}

/// End to end through the real binary: `schedule` writes a unit, `doctor` reads it back and
/// agrees it is current; a comment-only edit stays `ok`; a directive edit — the shape of a unit
/// written by an older binary — warns and exits 3. Linux only, because the unit directory is
/// derived from `$HOME` there and the classification itself is unit-tested on every platform.
#[cfg(target_os = "linux")]
#[test]
fn doctor_compares_the_installed_unit_with_what_this_binary_would_write() {
    let (dir, config) = fixture(SCHEDULABLE_CONFIG);
    let home = dir.path().join("home");
    let unit_dir = home.join(".config/systemd/user");

    // `--unit-dir` so nothing is reloaded or armed; it is the directory `doctor` reads from.
    let (_out, stderr, code) = run_code(&[
        "schedule",
        "-n",
        "dot-files",
    ...
    assert_eq!(code, 0, "schedule failed:\n{stderr}");
    let service = unit_dir.join("rusticprofile-dot-files.service");
    let original = std::fs::read_to_string(&service).expect("the unit schedule wrote");

    let (severity, finding, _) = units_current(&home, &config, &[]);
    assert_eq!(
        severity, "ok",
        "a freshly written unit must be current: {finding}"
    );
    assert!(finding.contains("match"), "{finding}");

    // Comment-only drift — what 0.2.46 did to every unit. Must not warn.
    std::fs::write(
        &service,
        original.replacen(
            "# Generated by rusticprofile.",
    ...
    .unwrap();
    let (severity, finding, _) = units_current(&home, &config, &[]);
    assert_eq!(severity, "ok", "comment-only drift must stay ok: {finding}");
    assert!(finding.contains("only comments differ"), "{finding}");

    // A directive difference — here, a unit from before the retry reached systemd (0.2.16).
    // Edit the `ExecStart=` line itself and assert the edit landed there: `--background` also
    // appears in a comment above it, and a first version of this test removed that one instead
    // — a comment-only change, which the check then correctly reported as `ok`.
    let stale: String = original
        .lines()
        .map(|l| {
            if l.starts_with("ExecStart=") {
                l.replace(" --background", "")
            } else {
                l.to_string()
            }
        })
    ...
    assert!(
        exec(&original).unwrap().contains("--background"),
        "precondition: {original}"
    );
    assert!(
        !exec(&stale).unwrap().contains("--background"),
        "the sabotage must apply"
    );
    std::fs::write(&service, stale).unwrap();
    let (severity, finding, code) = units_current(&home, &config, &[]);
    assert_eq!(severity, "warn", "a changed directive must warn: {finding}");
    assert!(
        finding.contains("--background"),
        "must name the line: {finding}"
    );
    assert_eq!(code, 3, "any warning sets doctor's exit code");

    // Simulating another host: the installed units are this machine's, so no verdict.
    let (severity, finding, _) = units_current(&home, &config, &["--as-host", "host-z"]);
    assert_eq!(severity, "unknown", "{finding}");
}
